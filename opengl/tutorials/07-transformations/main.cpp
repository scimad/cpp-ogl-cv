...
int main() {
  ...
  // zr::log("VEC = > " + std::to_string(vec.x) + ", " + std::to_string(vec.y) + ", " + std::to_string(vec.z));
  ...
  while (!glfwWindowShouldClose(window)) {
    ...
    trans = glm::mat4(1.0f);
    trans = glm::translate(trans, glm::vec3(cos(anglular_velocity*timeValue), sin(anglular_velocity*timeValue), 1.0f));
    trans = glm::scale(trans, glm::vec3(sin(timeValue), sin(timeValue), 1));
    trans = glm::rotate(trans, timeValue*int(0.5*anglular_velocity), glm::vec3(0.0f, 0.0f, 1.0f));
